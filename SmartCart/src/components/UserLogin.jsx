    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.error_code === 'WRONG_LOGIN_PORTAL') {
          setMessage("⚠️ " + data.message + " Please use the Vendor Login portal instead.");
        } else {
  ...
  return (
    <div style={{
      ...
      <form onSubmit={handleSubmit} style={{
        ...
        <h2 style={{ color: '#473472', fontWeight: 800, marginBottom: 8 }}>User Login</h2>
        <p style={{ color: '#666', fontSize: 14, marginBottom: 24, textAlign: 'center' }}>
          Sign in to browse products, shop, and manage your orders
        </p>
        ...
        <div style={{ marginTop: 20, textAlign: 'center', width: '100%' }}>
          <span style={{ color: '#666', marginRight: 8 }}>Don't have an account?</span>
        ...
        <div style={{ 
          ...
          <p style={{ color: '#888', fontSize: 13, marginBottom: 8 }}>Are you a vendor?</p>
          <Link 
            to="/login/vendor" 
            style={{

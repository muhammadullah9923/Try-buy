...
export default function AdminReport() {
  ...
  return (
    <section style={{ maxWidth: 900, margin: "2rem auto", background: "#fff", padding: 24, borderRadius: 12, boxShadow: "0 2px 12px rgba(0,0,0,0.07)" }}>
      <h2 style={{ color: "#473472" }}>Admin Sales & Commission Report</h2>
      ...
      {report && (
        <div>
          <div><b>Total Sales:</b> Rs. {report.total_sales}</div>
          <div><b>Total Commission:</b> Rs. {report.total_commission}</div>
          <div><b>Total Orders:</b> {report.orders.length}</div>

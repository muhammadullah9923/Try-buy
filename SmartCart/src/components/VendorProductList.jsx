...
export default function VendorProductList({ vendorId }) {
  ...
  useEffect(() => {
    if (!vendorId) return;
    setLoading(true);
    fetch(`http://127.0.0.1:8000/api/vendor/${vendorId}/products/`)
      .then(r => r.json())
      .then(data => {
          ...
          <tbody>
            {products.map(p => (
              <tr key={p.id}>
                ...
                <td>{p.occasion || "-"}</td>
                <td>{p.description}</td>
                <td>{p.image ? <img src={p.image.startsWith('http') ? p.image : `http://127.0.0.1:8000${p.image}`} alt={p.name} style={{ width: 60, borderRadius: 6 }} /> : "-"}</td>
